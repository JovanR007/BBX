            ...
            <div className="container mx-auto p-4 py-12 grid gap-12 lg:grid-cols-3">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="grid gap-4">
                        {participations && participations.length > 0 ? participations.map(p => (
                            <Link href={`/t/${p.tournaments.id}`} key={p.id} className="block group">
                                <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 flex items-center justify-between hover:border-cyan-500/50 hover:bg-slate-900/80 transition-all">
                                    <div>
                                        <div className="text-xs text-slate-500 font-mono mb-1 flex items-center gap-2">
                                            ...
                                            {new Date(p.tournaments.created_at).toLocaleDateString()}
                                            <span className={`px-1.5 py-0.5 rounded text-[10px] uppercase font-bold ${p.tournaments.status === 'completed' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'
                                                }`}>
                                                {p.tournaments.status}
                                            </span>
                                        </div>
                        ...
                        {recentMatches.map(m => (
                            <div key={m.id} className="bg-slate-900 border border-slate-800 rounded-lg p-3 text-sm">
                                ...
                                <div className="flex justify-between items-center px-2">
                                    <span className={m.winner_id && participations.map(p => p.id).includes(m.winner_id) ? "text-green-400 font-bold" : "text-white"}>
                                        {m.score_a}
                                    </span>
                                    <span className="text-slate-600 font-black italic">VS</span>
                                    <span className={m.winner_id && !participations.map(p => p.id).includes(m.winner_id) && m.winner_id ? "text-red-400" : "text-white"}>
                                        {m.score_b}
                                    </span>
                                </div>
                            </div>
