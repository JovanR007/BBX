                        ...
                        {loading ? (
                            ...
                            <>
                                ...
                                <ConfirmationModal
                                    ...
                                    onConfirm={async () => {
                                        const form = document.querySelector('form[class="space-y-2"]'); // Simple lookup for now
...
function AdvanceRoundButton({ tournamentId }) {
    const { toast } = useToast();
...
function RegistrationSection({ tournament, participants, loading, fetchData, tournamentId }) {
    const { toast } = useToast();
...
function LateEntryForm({ tournamentId, refresh }) {
...
export function ConcludeTournamentSection({ tournamentId }) {
    const { toast } = useToast();
    const [isOpen, setIsOpen] = useState(false);
    const [loading, setLoading] = useState(false);

    async function handleConclude(e) {
        e.preventDefault();
        setLoading(true);
