...
export async function generateTopCut(tournamentId: string, requestedCutSize: number) {
    try {
        ...
        try {
            ...
            console.error("Error generating pairings in Top Cut:", err);
            throw new Error(`Critical Error in Pairing Loop: ${err.message}`);
        }
        ...
        console.error("Critical error in generateTopCut:", err);
        throw new Error(err.message || "Unknown error in generation core");
    }
}
