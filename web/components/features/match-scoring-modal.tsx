...
export function MatchScoringModal({ isOpen, onClose, match, participants, refresh }: { isOpen: boolean; onClose: () => void; match: any; participants: any; refresh: () => void }) {
    const { toast } = useToast();
    const [scoreA, setScoreA] = useState(0);
    const [scoreB, setScoreB] = useState(0);
    const [warningsA, setWarningsA] = useState(0);
    const [warningsB, setWarningsB] = useState(0);
    const [lastMove, setLastMove] = useState<any>(null); // { player: 'A'|'B', type: 'spin'|'over'|'burst'|'pocket' }
    ...
    const syncScore = async (newA: number, newB: number) => {
    ...
    async function handleOverrideSubmit(e: any) {
        e.preventDefault();
        setSubmitting(true);
...
function ScoreBtn({ onClick, label, pts, color, disabled }: { onClick: () => void; label: string; pts: number; color: string; disabled: boolean }) {
    const colorStyles = {
...
function ScoreBtnMobile({ onClick, label, color, side, disabled }: { onClick: () => void; label: string; color: string; side: string; disabled: boolean }) {
    const colorStyles = {
