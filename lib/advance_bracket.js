...
function makeByeMatch({ tournamentId, roundId, bracketRound, matchNumber, aId, target }) {
    return {
...
export async function advanceBracket(tournamentId) {
    ...
    // We check if ANY match in the current highest round is NOT complete
    ...
    // Round 1 matches = cutSize / 2
    // Round K matches = cutSize / (2^K)
    // The "ideal range" of match numbers in Round K is 1 .. (cutSize / 2^K).

    // We are currently at `currentRoundNum`.
    // The max match number in this round should be cutSize / 2^(currentRoundNum).
    ...
    if (isFinals) {
        ...
        // Assuming prevMatches are sorted by match_number or we can rely on map lookup
        ...
        if (existing && existing.length > 0) {
            throw new Error(`Round ${nextRoundNum} already exists.`);
        }
    ...
    const matchesToInsert = [];
    const target = 4; // Default target points for non-finals matches
    ...
    for (let i = 1; i <= nextRoundSize; i++) {
        ...
        // Next Match 1 comes from Prev Match 1 & 2
        // Next Match 2 comes from Prev Match 3 & 4
        const slotA = 2 * i - 1;
        const slotB = 2 * i;
        const currentMatchNum = i; // Strict Grid Position
        ...
        if (newMatch) {
            matchesToInsert.push({
                ...
                stage: "top_cut",
                bracket_round: nextRoundNum,
                match_number: currentMatchNum, // Preserve Grid Position
                target_points: target,
                ...newMatch
            });
        }
